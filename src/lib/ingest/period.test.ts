...
function ym(year: number, month: number): YearMonth {
  ...
  if (!value) throw new Error(`нет месяца ${month}`);

  return { year, month: value };
}

describe("buildPeriod", () => {
  it("формирует месяц в формате legacy", () => {
  ...
  it("отвергает два месяца — это не месяц и не квартал", () => {
  ...
  it("отвергает три месяца, не составляющих календарный квартал", () => {
  ...
  it("отвергает месяцы разных лет раньше, чем смотрит на кварталы", () => {
...
describe("parseAllegroDate", () => {
  ...
  it("отвергает дату без времени", () => {
    expect(parseAllegroDate("31.07.2026")).toBeNull();
  });
});

describe("parseCdiscountDate", () => {
  it("разбирает и текстовую дату, и дату-ячейку из XLSX", () => {
    expect(parseCdiscountDate("2026-06-01")).toEqual(ym(2026, 6));
    expect(parseCdiscountDate("2026-06-01 00:00:00")).toEqual(ym(2026, 6));
  });

  it("не принимает время, отличное от полуночи — это уже не дата учёта", () => {
    expect(parseCdiscountDate("2026-06-01 13:00:00")).toBeNull();
  });
});

describe("parseShopifyDate", () => {
  it("берёт календарную дату как записана, без пересчёта пояса", () => {
    // Пересчёт в UTC увёл бы транзакцию в июнь и сломал бы период файла.
...
describe("parseAmazonMonthlyDate", () => {
  ...
  it("разбирает немецкий числовой формат", () => {
    expect(parseAmazonMonthlyDate("30.04.2026 22:42:06 UTC")).toEqual(ym(2026, 4));
  });

  it("отвергает неизвестное название месяца", () => {
