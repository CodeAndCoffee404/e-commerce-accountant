...
> **Роль подключения не должна иметь `BYPASSRLS`.** Разделение компаний
> держится на политиках row-level security, а роль с этим атрибутом проходит
> мимо любой политики — весь слой становится бесполезным, и снаружи это
> никак не видно: приложение работает ровно так же. Атрибут выдаётся роли, а
> не таблице, поэтому ни одна миграция его не исправит.
>
> Проверить (на любой из двух баз):
>
> ```sql
> select rolname, rolsuper, rolbypassrls from pg_roles where rolname = current_user;
> ```
>
> Оба флага должны быть `f`. Это же утверждает тест
> `tests/tenant-isolation.test.ts` — он падает, если подключение идёт ролью,
> которой политики не писаны.
...
> **Если миграция падает, `drizzle-kit migrate` не говорит почему.** Он
> печатает два безобидных уведомления про свои же служебные таблицы и выходит
> с кодом 1 — в логе остаётся только `Command "npm run vercel-build" exited
> with 1`. Прочитать причину больше негде: боевая строка подключения помечена
> Sensitive, повторить запуск руками нельзя.
>
> Поэтому `db:migrate` — это `drizzle-kit migrate || node scripts/migrate.mjs`.
> Применяет по-прежнему CLI; скрипт запускается только после его отказа, теми
> же миграциями по тому же журналу, и печатает то, что сказала Postgres:
> запрос, `cause`, код, `detail`, `position`. Упавшая миграция откатывается
> целиком, поэтому вторая попытка ничего не стоит.
>
> Миграции идут **одной транзакцией на весь прогон**. Значит упавший деплой
> оставляет базу ровно в том состоянии, в каком она была: ничего не применено
> наполовину, и прод продолжает работать на предыдущей версии.
...
Этот же вход **делает вошедшего супер-админом** — человеком над компаниями,
который видит их список и может завести новую. Признак пишется в строку
пользователя (`users.is_super_admin`) и читается на каждый запрос, поэтому
снять его — правка строки, а не ожидание, пока истечёт сессия.

Из этого следует одно практическое требование: **`AUTH_BOOTSTRAP_EMAILS` должен
быть выставлен в проде хотя бы на один вход**. Если переменную убрали после
первого запуска, супер-админа сделать больше нечем — админ-зона окажется без
входа, а завести её из базы нельзя, потому что строка подключения к проду
не читается.
